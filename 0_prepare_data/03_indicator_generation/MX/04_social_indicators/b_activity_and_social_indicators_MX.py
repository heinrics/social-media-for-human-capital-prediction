...
input_dir = base_path + '/01_data/MX/02-processed-data/social/'
...
def load_data(time_split, filter_date_end, week, out_dir):
    ...
    social_df = pd.read_csv(input_dir + 'tweet-level-indicators-MX.csv',
                            sep=';',
                            encoding='utf-8')
    ...
    if week <= 9:
        if filter_date_end:
            social_df = social_df[(social_df['created_at_tm'] > time_split) &
                                  (social_df['created_at_tm'] < filter_date_end)]
        else:
            social_df = social_df[(social_df['created_at_tm'] > time_split)]
    ...
    if (week > 9) and filter_date_end and time_split:
        ...
        #                       (social_df['created_at_tm'] > time_split)]


    # Load and update correct user county data
    social_df.rename(columns={'mun_id_user': 'mun_id_user_full',
...
def user_social(df, out_path):
    ...
    user_social_df['account_created_at_struct'] = user_social_df['account_created_at']\
    ...
    user_social_df['account_age_last_tweet_year'] = user_social_df.apply((lambda x: (time.mktime(x['created_at_tm']) -
                                                                                     time.mktime(x['account_created_at_struct'])) / 60 / 60 / 24 / 365),
                                                                         axis=1)
    ...
    # Export user level ############################################################
    ################################################################################
    user_social_export_df = user_social_df[['user_id',
                                            'tweet_id',
                                            'tweet_created_at',
                                            'mun_id_user',
                                            'mun_id_tweet',
    ...
    # Aggregate on municipality level ##########################################
    mun_social_df = user_social_export_df[['mun_id_user',
                                           'favourites_per_statuses',
                                           'statuses_count_per_year',
                                           'account_age_absolute_year',
                                           'listed_count',
...
def compute_indicators(filter_date_start, filter_date_end, week, out_dir):
    ...
    user_social(df, out_dir + 'mun-level-social-indicators-MX.csv')
