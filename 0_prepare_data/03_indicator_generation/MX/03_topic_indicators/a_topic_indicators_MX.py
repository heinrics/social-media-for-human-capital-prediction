...
def load_data(time_split, filter_date_end, week, out_dir):
    ...
    topic_df = pd.read_csv(input_path + "/topic-data-mx.csv",
                           dtype={'mun_id_user': str,
                                  'mun_id_tweet': str},
                           low_memory=False,
                           lineterminator='\n',
                           encoding="utf-8",
    ...
    if week <= 9:
        if filter_date_end:
            topic_df = topic_df[(topic_df['created_at_tm'] > time_split) &
                                (topic_df['created_at_tm'] < filter_date_end)]
        else:
            topic_df = topic_df[(topic_df['created_at_tm'] > time_split)]
    ...
    if (week > 9) and filter_date_end and time_split:
        ...
        #                     (topic_df['created_at_tm'] > time_split)]

    # Load and update correct user county data
    topic_df.rename(columns={'mun_id_user': 'mun_id_user_full',
...
def user_topics(df, out_path):
    ...
    # Aggregate on user level ##################################################
    user_topic_df = df.groupby(['user_id', 'mun_id_user']) \
                      [topic_list]\
                      .mean()\
                      .reset_index()
    ...
    county_topic_df = user_topic_df.groupby('mun_id_user') \
                                   [topic_list] \
                                   .mean() \
                                   .reset_index()
...
def compute_indicators(filter_date_start, filter_date_end, week, out_dir):
    ...
    user_topics(df, out_dir + 'mun-level-topic-indicators-MX.csv')
