...
if __name__ == '__main__':
    ...
    old_translation_df = pd.read_csv(input_path + "/translation-data-old.csv",
                                     lineterminator='\n',
                                     sep=";")

    missing_tweets = df[~df['id'].isin(old_translation_df['id'])]


    ddf = dd.from_pandas(missing_tweets, chunksize=50)
    print("data loaded")
...
if __name__ == '__main__':
    ...
    def translate_es_to_en(part_df):
        ...
        try:
            ...
            # print('Begin batch with device: ' + str(selected_device_idx))
            ...
            # translation = [tokenizer.decode(t.detach(), skip_special_tokens=True) for t in translation_vec.detach().to('cpu')]
...
if __name__ == '__main__':
    ...
    translation_df.rename(columns={'en_trans': 'en_translation'}, inplace=True)

    updated_translation_df = pd.concat([old_translation_df, translation_df])
    updated_translation_df = updated_translation_df[updated_translation_df['id'].isin(df['id'])]

    # Save topic dataset
    # TODO: uncomment when processing:
    # updated_translation_df.to_csv(output_path + "/translation-data.csv",
    #                               encoding="utf-8",
    #                               index=False,
    #                               sep=";")
    print("exported data")
