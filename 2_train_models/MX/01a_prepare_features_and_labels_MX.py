...
# Import indicator data (full data = week 9)
df = pd.read_csv(input_path + r"\weeks\week_09\mun-level-complete-MX-09.csv",
                 sep=";")
df = df.rename(columns={"mun_id_user": "GEOID"})
len(df)
...
edu = pd.read_excel(edu_path + "/education-data.xlsx",
                    dtype={'GEOID': 'string'})
...
def make_all_hists(df, X_cols):
    ...
    make_hists(df[X_cols[54:]], 6, 3, 15, 10)
    make_hists(df[X_cols[72:]], 5, 1, 15, 10)
...
for col_group in [pop_cols, count_cols, error_cols, topic_cols, sentiment_cols, network_cols]:
    ...
    for col in col_group:
        ...
        pct_pb = df["edu_postbasic"].corr(df[col]).round(3)
        print(f"{col}: {pct_ys} for years of schooling; {pct_pb} for share postbasic")
