...
################################################################################
#
# Prepare features per week for MX
# -----------------------------------------------------------
...
geo = geopandas.read_file(geo_path + "/mex_admbnda_adm2_govmex_20210618.shp")
geo = geo.rename(columns={"ADM2_PCODE": "GEOID"})
geo = geo[["GEOID", "geometry"]]
len(geo)

################################################################################
# Create feature data for each week
################################################################################
...
for week in range(1, 10):
    ...
    # Import data
    i = str(week).zfill(2)
    df = pd.read_csv(input_path + fr"\weeks\week_{i}\mun-level-complete-MX-{i}.csv",
                     sep=";")
...
for day in range(1, 8):
    ...
    # Import data
    i = str(day).zfill(2)
    df = pd.read_csv(input_path + fr"\days\day_{i}\mun-level-complete-MX-{i}.csv",
                     sep=";")
    df = df.rename(columns={"mun_id_user": "GEOID"})
