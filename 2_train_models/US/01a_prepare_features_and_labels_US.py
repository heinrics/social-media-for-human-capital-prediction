...
#-------------------------------------------------------------------------------
################################################################################
# Prepare features for prediction models at US county levels:
# -----------------------------------------------------------
# - Merges county-level features with education and geodata
# - Computes weighted cluster averages for each county cluster
# - Inspects distributions of variables and takes log of right-skewed variables
# - Imputes values for counties with 0 tweets based on cluster averages
# - Handles outliers
# - Exports features and labels
# - Note: Performs the same feature preparations as the function prepare_features,
#   but also prints intermediary results and plots distributions
################################################################################
#-------------------------------------------------------------------------------
...
from prepare_features import make_hists
...
# Import indicator, validation and geodata
#-------------------------------------------------------------------------------

# Import indicator data
df = pd.read_csv(input_path + fr"\weeks\week_09\county-level-complete-US-09.csv",
                 sep=";", dtype={"county_id_user": str})
df = df.rename(columns={"county_id_user": "GEOID"})
len(df)

# Import and add geodata
geo_df = geopandas.read_file(geo_path + r"\cb_2018_us_county_500k.shp",
                             dtype={"GEOID": str})
geo_df = geo_df[["GEOID", "geometry"]]

df = df.merge(geo_df, on="GEOID")
df = geopandas.GeoDataFrame(df)
df = df.to_crs("EPSG:5071")
len(df)

# Import education data
edu = pd.read_csv(edu_path + "/education-data.csv",
                  sep=";", dtype={'GEOID': 'string'})
edu = edu.drop(columns="NAME")
len(edu)
...
# Inspect and transform features for df and cluster df
# ------------------------------------------------------------------------------

# Make histograms of all variables for inspection
len(X_cols)
def make_all_hists(df, X_cols):

    # Create all histograms based on nr of columns
    make_hists(df[X_cols[:]], 6, 3, 15, 10)
    make_hists(df[X_cols[18:]], 6, 3, 15, 10)
    make_hists(df[X_cols[36:]], 6, 3, 15, 10)
    make_hists(df[X_cols[54:]], 4, 4, 8, 10)
make_all_hists(df, X_cols)
...
# Inspect results
make_all_hists(df_counties, X_cols)
make_all_hists(df_clusters, X_cols)
...
df_clusters.columns = list(df_clusters.columns[:1]) \
                      + list(["cluster_" + x for x in df_clusters.columns[1:]])
...
for col in tweet_cols + topic_cols + error_cols + sentiment_cols + network_cols:
    impute_na(df=df, yvar=col, Xvars=Xvars,
              model=enet_model, miss_indicator="zero_tweets")

# Inspect results
df[X_cols].isna().sum()
df.loc[df["zero_tweets"]==1, count_cols + pop_cols + tweet_cols + sentiment_cols + network_cols]
make_all_hists(df_counties, X_cols)
...
################################################################################
# Inspect variables and correlations
################################################################################

# Merge with validation data
df = df.merge(edu, on="GEOID")
len(df)

# Select two main outcome variables: edu_years_schooling and edu_postbasic
df["edu_years_schooling"].describe()
df["edu_bachelor"].describe()

# Check pairwise correlation with y
# ------------------------------------------------------------------------------

for col_group in [pop_cols, count_cols, error_cols, topic_cols, sentiment_cols, network_cols]:
    print(f"\n************************************************************")
    print("\nMun level correlations:")
    for col in col_group:
        pct_ys = df["edu_years_schooling"].corr(df[col]).round(3)
        pct_pb = df["edu_bachelor"].corr(df[col]).round(3)
        print(f"{col}: {pct_ys} for years of schooling; {pct_pb} for share bachelor")

    print("\nCluster level correlations:")
    for col in ["cluster_" + x for x in col_group]:
        pct_ys = df["edu_years_schooling"].corr(df[col]).round(3)
        pct_pb = df["edu_bachelor"].corr(df[col]).round(3)
        print(f"{col}: {pct_ys} for years of schooling; {pct_pb} for share bachelor")


# Create heatmap of selected x variables
selected_cols = ["log_user_density", "log_tweet_density",
                 "log_avg_chars_tweet", "log_emoji_per_tweet",
                 "log_error_per_char",
                 "log_science_&_technology", "log_relationships",
                 "log_ref_pagerank"]
...
label_data = df[edu.columns[1:]]
list(label_data.columns)
label_data.to_csv(output_path + r"\labels.csv", encoding="UTF8", sep=";")

feature_data = df.drop(columns=label_data.columns)
list(feature_data.columns)
feature_data.to_csv(output_path + r"\features.csv", encoding="UTF8", sep=";")
